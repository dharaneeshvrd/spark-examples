...
q1_out = gpay_df.groupBy(gpay_df.Applicable_Manufacturer_or_Applicable_GPO_Making_Payment_Name, gpay_df.Physician_Specialty).agg({'Total_Amount_of_Payment_USDollars': 'sum'}).repartition(1).write.mode('overwrite').json('/tmp/spark_poc1/gpay/q1_out')
...
q2_result = df1.join(df2, condition) \
            ...
            .repartition(1) \
            .write.mode('overwrite').json('/tmp/spark_poc1/gpay/q2_out')
