...
gpay = sc.textFile('hdfs://jana-hadoop-mgr-1:8020/tmp/OP_DTL_GNRL_PGYR2017_P06292018_edited_for_spark.csv', sc.defaultParallelism)
...
st = time.time()
gpay_df.write.parquet('hdfs://jana-hadoop-mgr-1:8020/tmp/parq_out1')
et = time.time()
print "time taken:", et-st

# Creating a seperate column for month
...
# Writing the df as parquet by partiton the dataframe by month column
st = time.time()
gpay_df_with_mon.repartition('Month_for_Partition').write.partitionBy('Month_for_Partition').parquet('hdfs://jana-hadoop-mgr-1:8020/tmp/parq_out1_mon_par')
et = time.time()
print "time taken:", et-st
